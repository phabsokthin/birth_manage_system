...
<div class="content-wrapper p-4">
    ...
    <form id="delete-form" method="POST" action="{{ route('communes.deleteall') }}">
        @csrf
        @method('DELETE')
            ...
            <table class="table table-striped my-3 table-bordered " id="Datatable">
                <thead class="sidebar-dark-primary text-light" style="width: 100px">
                    <tr>
                        ...
                        <th scope="col">ស្រុក(ខ្មែរ)</th>
                        <th scope="col">ស្រុក(អងគ្លេស)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row">
                            <input type="checkbox" name="ids[]" value="{{ $com->commune_id }}"
                                class="delete-checkbox" />
                        </th>
                        <td>{{ $com->district->district_kh_name }}</td>
                        <td>{{ $com->district->district_en_name }}</td>
                        <td>{{ $com->commune_kh_name }}</td>
                        <td>{{ $com->commune_en_name }}</td>
                        <td>{{ $com->commune_code }}</td>
                        <!-- <td>{{ $com->status }}</td> -->
                        <td>
                            <div
                                class="{{ $com->status == 1 ? 'btn btn-success btn-xs rounded-pill' : 'btn btn-danger btn-xs rounded-pill' }}">
                                {{ $com->status == 1 ? 'Active' : 'Inactive' }}
                            </div>
                        </td>
                        <td class="text-center align-middle">
                            <a href="view-details-url" class="btn btn-warning btn-sm p-1" title="View Details"
                                data-bs-toggle="modal" data-bs-target="#ModalDetail{{ $com->commune_id }}">
                            ...
                            <div style="display: inline-block; margin-left: 5px;" class="">
                                <a href="#" class="edit btn btn-secondary btn-sm p-1" data-bs-toggle="modal"
                                    data-bs-target="#ModalEdit{{ $com->commune_id }}">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="#" class="edit btn btn-danger btn-sm p-1" data-bs-toggle="modal"
                                    data-bs-target="#ModalDelete{{ $com->commune_id }}">
...
<!-- including modal -->
@include('commune.modal.create')
@include('commune.modal.update')
@include('commune.modal.detail')
@include('commune.modal.delete')
@include('commune.modal.deleteall')
